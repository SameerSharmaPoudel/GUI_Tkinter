...
class widgets(Frame):
    ...
    def __init__(self, window, picks=[]):
      ...
      label = Label(lf, text='dAlpha', width=6)
      label.grid(row=0, column=1)
      
      var = IntVar()
      var.set(0.1)
      self.dAlpha = Entry(lf, textvariable=var, width=6)
      self.dAlpha.grid(row=0, column=2) 
      ...
      for j in range(len(self.curvenames)): 
          ...
          self.curves.append(self.checks)
    ...
    def state(self): 
      ...
      dAlpha = float(self.dAlpha.get())
      #print(dAlpha)
      base_curve = list(map((lambda ent: float(ent.get())), self.base_curve))
      #print(base_curve)
      result_cvs = []
      result_names =  []

      for cu, curve in enumerate(self.curves):
          curve = list(map((lambda chk: chk.get()), curve))
          bc = list(map((lambda ent: float(ent.get())), self.base_curve))
          save_flag=0  
          for ch, check in enumerate(curve):                
              if check ==1:
                  bc[ch] = bc[ch] + dAlpha    #only one alpha value for a curve can be changed !
                  save_flag=1
                      
          if save_flag==1:     
             result_cvs.append(bc)
      ...
      print(base_curve)
      print(dAlpha)                               
      return dAlpha, base_curve, result_cvs, result_names
...
def allstates():
    ...
    dAlpha, base_curve, curves, curve_names = lng.state()  
    print(dAlpha)
    print(base_curve)
    ...
    for l, curve in enumerate(curves):
        ...
        alpha, a = curve[:-1], curve[-1]
        R_base, Rb_base = get_R_vs_Theta_Rb_values(base_curve[:-1], base_curve[-1])
        stress_base = get_UniaxYieldStress_vs_Theta_values(base_curve[:-1], base_curve[-1], Y_0=1.0, NormalizeAlpha=False)
        yl_base = get_stress_points_in_Yieldlocus(base_curve[:-1], base_curve[-1],shear=0)
        ...
        sens_stress = np.absolute( stress - stress_base)/ dAlpha
        sens_R = np.absolute (R - R_base)/ dAlpha        
        sens_yl = np.sqrt( ((yl_base[0][0][0]-yl[0][0][0])**2) +  ((yl_base[0][1][0]-yl[0][1][0])**2) )  / dAlpha         
        sens_yl_315to135 = []
        sens_yl_315to135.extend(sens_yl[315:360])
        sens_yl_315to135.extend(sens_yl[0:136])
        #print(np.shape(sens_yl_315to135))
        
        angle_plane_strain_1 = yl[0][12]
        angle_plane_strain_2 = yl[0][13]
        angle_sigma_biax = yl[0][14] 
        ...
        v = [angle_plane_strain_1, angle_plane_strain_2, angle_sigma_biax]        
        values.append(v)
        
    display(frame1, values, curve_names, ['angle_PS1x', 'angle_PS2y', 'angle_Sbiax'])  
...
def normalize():
    ...
    alpha_base, a_base = base_curve[:-1], base_curve[-1]
    factor_base = ( (abs((2.0*alpha_base[0]+alpha_base[1])/3.0))**a_base + (abs((2.0*alpha_base[2]-2.0*alpha_base[3])/3.0))**a_base + (abs((4.0*alpha_base[4]-alpha_base[5])/3.0))**a_base) / 2.0
    factor_base =  round(float(factor_base ** (1.0/a_base)),4)
    alpha_base = [i/factor_base for i in alpha_base]  # here the normalized values
    ...
    for l, curve in enumerate(curves):
        ...
        R_base, Rb_base = get_R_vs_Theta_Rb_values(alpha_base, a_base)
        stress_base = get_UniaxYieldStress_vs_Theta_values(alpha_base, a_base, Y_0=1.0, NormalizeAlpha=False)
        yl_base = get_stress_points_in_Yieldlocus(alpha_base, a_base,shear=0)
...
if __name__ == '__main__':
    ...
    lng = widgets(frame1, ['a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8', 'e'])
